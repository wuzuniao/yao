...
CREATE TABLE `checkin_plans` (
  ...
  `end_date` DATE NOT NULL COMMENT '计划结束日期（end_mode=1/2时存9999-12-31哨兵，按模式分支处理）',
  `repeat_weekdays` TINYINT NOT NULL DEFAULT 127 COMMENT '重复星期位掩码：bit0=周一…bit6=周日，127=每天，31=工作日，96=周末',
  `end_mode` TINYINT NOT NULL DEFAULT 0 COMMENT '结束方式：0-按end_date，1-按打卡总次数，2-长期不结束（end_date存9999-12-31哨兵）',
  `total_target_count` INT NULL COMMENT '目标打卡总次数（end_mode=1时生效，累计按时间点+日期去重计数，达标后自动置status=0）',
  `status` TINYINT NOT NULL DEFAULT 1 COMMENT '状态：1-进行中，2-暂停，0-已结束',
  `priority` INT NOT NULL DEFAULT 3 COMMENT '优先级：数字越小优先级越高（范围0-3，默认3）',
...
CREATE TABLE `plan_notification_times` (
  ...
  `followup_count` INT NOT NULL DEFAULT 3 COMMENT '提醒总次数（含准时）：3=默认三段式（准时+10分钟+1小时或中点），1/2=自定义等间隔',
  `followup_interval_min` INT NOT NULL DEFAULT 10 COMMENT '自定义等间隔分钟（5-60，followup_count=3时不生效）',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
...
--   - App推送：channel_type='App推送'，每用户仅一行，多设备共存于同一 JSON 数组中
...
CREATE TABLE `notification_channels` (
  ...
  `channel_type` VARCHAR(20) NOT NULL COMMENT '通知类型（站内信、邮件、微信、App推送）',
  `channel_value` TEXT NOT NULL COMMENT '通知值（站内信存用户ID；邮件存 SMTP JSON；微信存额度 JSON；App推送 存设备token数组 JSON）',
...
CREATE TABLE `notification_logs` (
  ...
  `trigger_type` TINYINT NOT NULL DEFAULT 0 COMMENT '触发类型：0-准时；默认三段式（followup_count=3）1-超10分钟、2-1小时或中点；自定义等间隔（count=2）1-提醒时间+间隔分钟',
